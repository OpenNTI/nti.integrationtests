...
from .context import DelegateContext
...
def read_config(config_file, process_args=True):
	...
	context.base_path = os.path.expanduser(get_option(config, name="base_path",
													  default=config_dir))
	...
	if process_args:
		context.script_setup = resolve(context.script_setup) \
		if hasattr(context, "script_setup") else noop

		context.script_teardown = resolve(context.script_teardown) \
		if hasattr(context, "script_teardown") else noop

		context.script_subscriber = resolve(context.script_subscriber) \
		if hasattr(context, "script_subscriber") else None
	...
	for num, section in enumerate(config.sections()):
		...
		if process_args:
			...
			delegate.target_args = eval(delegate.target_args) \
			if delegate.target_args else delegate.target_args
			
		# resolve setup/teardown
		if process_args:
			delegate.setup = resolve(delegate.setup) \
			if hasattr(delegate, "setup") else noop

			delegate.teardown = resolve(delegate.teardown) \
			if hasattr(delegate, "teardown") else noop
