...
class VariableErrorJsonPutTests(V3TestCase):
	...
	def _run_test(self, URL, data, code, body, string, password=None):
		...
		tester.putTest(URL, data=data, password=password, bodyDataExtracter=bodyDataExtracter, fmt=self.json)
		modifiedTimeID = tester.getLastModified(URL)
	...
	def test_Server401JsonFormatIncorrectPasswordPutTestCase(self):
		self._run_test(	self.URL_json, data=self.put_questions, password=self.incorrectPassword,\
	...
	def test_Server500JsonFormatWrongDatatypePutTestCase(self):
		self._run_test(self.URL_json, data=self.wrongInfo, code=self.WrongType, body=self.default_answer, string='WrongData')
