...
def create_friends_list(username, friends, list_name=None, server='localhost', port=8081, 
...
def create_test_friends_lists(users, server='localhost', port=8081):
    host = 'test.user.1@nextthought.com'
    users =['test.user.%s@nextthought.com' % s for s in range(2, users+1)]
    all_users = [host] + users
    for username in all_users:
        create_friends_list(username, all_users, server=server, port=port)
...
def simulate(users, containerId, entries=None, delay=2, server='localhost', port=8081,
             ...
             create_test_friends_lists=True):
    
    users = max(min(abs(users), 50), 2)
    entries = abs(entries) if entries else 50
            
    if create_test_friends_lists:
        create_test_friends_lists(users, server, port)
