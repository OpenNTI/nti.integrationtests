...
nti_run_general_purpose_tests --use_coverage --sync_changes >> $LOG 2>&1

# combine coverage data from general purpose tests
coverage combine >> $LOG 2>&1

# move file to be combined later
if [ -f $TMPDIR/.coverage ]; then
	rm -f $TMPDIR/.coverage.gpt
	mv $TMPDIR/.coverage $TMPDIR/.coverage.gpt
fi
